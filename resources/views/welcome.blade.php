...
@section('content')
                    ...
                    <div class="bg-white dark:bg-surface-dark p-2 rounded-2xl shadow-xl shadow-orange-500/10 max-w-xl">
                        <form action="{{ route('jelajah') }}" method="GET">
                            <div class="flex flex-col md:flex-row gap-2">
                                <div
                                    class="flex-1 flex items-center px-4 py-2 bg-gray-50 dark:bg-gray-800 rounded-xl group">
                                    <span
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
                ...
                <div class="flex gap-2 bg-white dark:bg-surface-dark p-1.5 rounded-xl shadow-inner">
                    <a href="{{ route('umkm.index') }}"
                        class="px-5 py-2 rounded-lg bg-gradient-to-r from-primary-orange to-primary-green text-white text-sm font-bold shadow-md">Lihat
                        Semua</a>
                </div>
            </div>
                ...
                @forelse($featuredUmkms as $umkm)
                    @php
                        $photoUrl = $umkm->primaryPhoto
                            ? asset($umkm->primaryPhoto->photo_path) // Sesuaikan path jika pakai storage/
                            : 'https://via.placeholder.com/400x300?text=UMKM';

                        // Cek fisik file agar aman
                        if ($umkm->primaryPhoto && !str_starts_with($umkm->primaryPhoto->photo_path, 'http')) {
                            ...
                            $photoUrl = $umkm->primaryPhoto->photo_path; // URL eksternal (seeder)
                        }
                    @endphp

                    <div
                        class="group bg-white dark:bg-surface-dark rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2">
                        <div class="relative h-48 overflow-hidden">
                            <div class="absolute top-4 left-4 z-10">
                                <span
                                    class="px-3 py-1.5 bg-gradient-to-r from-primary-orange to-orange-dark text-white text-xs font-bold rounded-full shadow-lg">
                        ...
                        <div class="p-6">
                            <div class="flex justify-between items-start mb-3">
                                <h3
                                    class="font-bold text-lg group-hover:text-primary-orange transition-colors line-clamp-1">
                                    {{ $umkm->nama_usaha }}
                                </h3>
                                <div
                                    class="flex items-center gap-1 bg-amber-50 dark:bg-amber-900/20 px-2 py-1 rounded-lg shrink-0">
    ...
    {{-- Bagian Gabung Mitra & Testimoni Biarkan Statis Dulu (Aman) --}}
    <section id="gabung-mitra" class="min-h-screen flex items-center justify-center py-20 bg-white dark:bg-surface-dark">
